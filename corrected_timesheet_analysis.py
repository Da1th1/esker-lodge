#!/usr/bin/env python3
"""
Corrected Esker Lodge Timesheet Analysis Script
Handles time format data properly and provides comprehensive analysis
"""
...
class CorrectedTimesheetAnalyzer:
    ...
    def clean_data(self):
        ...
        for col in hour_columns:
            if col in self.cleaned_df.columns:
                print(f"  Converting {col} from time format to hours...")
                self.cleaned_df[col + '_Hours'] = self.cleaned_df[col].apply(self.parse_time_to_hours)
        ...
        # Calculate Total Hours from components if Total Hours is missing
        if 'Total Hours_Hours' in self.cleaned_df.columns:
            # If Total Hours is zero/missing, calculate from other fields
            other_hour_cols = [col + '_Hours' for col in hour_columns if col != 'Total Hours' and col + '_Hours' in self.cleaned_df.columns]
            calculated_total = self.cleaned_df[other_hour_cols].sum(axis=1)
            
            # Use calculated total where Total Hours is zero
            mask = self.cleaned_df['Total Hours_Hours'] == 0
            self.cleaned_df.loc[mask, 'Total Hours_Hours'] = calculated_total[mask]
        ...
        print(f"📊 Records with Total Hours > 0: {(self.cleaned_df['Total Hours_Hours'] > 0).sum():,}")
    ...
    def basic_statistics(self):
        ...
        # Hours statistics
        total_hours_worked = self.cleaned_df['Total Hours_Hours'].sum()
        avg_hours_per_record = self.cleaned_df['Total Hours_Hours'].mean()
        records_with_hours = (self.cleaned_df['Total Hours_Hours'] > 0).sum()
        ...
        if total_hours_worked > 0:
            # Hours by department
            print(f"\n📊 HOURS BY DEPARTMENT")
            dept_hours = self.cleaned_df.groupby('Department_Clean')['Total Hours_Hours'].agg(['sum', 'mean', 'count']).round(2)
            ...
            staff_hours = self.cleaned_df.groupby('Name')['Total Hours_Hours'].agg(['sum', 'mean', 'count']).round(2)
    ...
    def detect_anomalies(self):
        ...
        high_hours = self.cleaned_df[self.cleaned_df['Total Hours_Hours'] > high_hours_threshold]
        low_hours = self.cleaned_df[(self.cleaned_df['Total Hours_Hours'] > 0) & (self.cleaned_df['Total Hours_Hours'] < low_hours_threshold)]
        
        print(f"🔺 HIGH HOURS (>{high_hours_threshold}h/week): {len(high_hours)} records")
        if len(high_hours) > 0:
            print(high_hours[['Name', 'Department_Clean', 'Total Hours_Hours', 'YearWeek']].head(10))
            anomalies['high_hours'] = high_hours
        
        print(f"\n🔻 LOW HOURS (<{low_hours_threshold}h/week, >0): {len(low_hours)} records")
        if len(low_hours) > 0:
            print(low_hours[['Name', 'Department_Clean', 'Total Hours_Hours', 'YearWeek']].head(10))
            anomalies['low_hours'] = low_hours
        
        # 2. Staff with repeated 0 hours across multiple weeks
        zero_hours_by_staff = self.cleaned_df[self.cleaned_df['Total Hours_Hours'] == 0].groupby('Name').size()
        ...
        missing_pay_rate = self.cleaned_df[
            ...
            (self.cleaned_df['Total Hours_Hours'] > 0)
        ]
        
        print(f"\n💰 MISSING PAY RATE WITH HOURS WORKED: {len(missing_pay_rate)} records")
        if len(missing_pay_rate) > 0:
            print(missing_pay_rate[['Name', 'Department_Clean', 'Total Hours_Hours', 'Pay Rate', 'YearWeek']].head(10))
        ...
        overtime_records = self.cleaned_df[self.cleaned_df['Total Hours_Hours'] > overtime_threshold]
    ...
    def weekly_analysis(self):
        ...
        weekly_dept = self.cleaned_df.groupby(['YearWeek', 'Department_Clean'])['Total Hours_Hours'].sum().reset_index()
        weekly_total = self.cleaned_df.groupby('YearWeek')['Total Hours_Hours'].sum()
        ...
        dept_totals = self.cleaned_df.groupby('Department_Clean')['Total Hours_Hours'].sum().sort_values(ascending=False)
    ...
    def save_results(self, staff_summary, dept_summary, contract_summary, anomalies, flags_df):
        ...
        # Main analysis file
        analysis_file = f"corrected_timesheet_analysis_{timestamp}.xlsx"
        with pd.ExcelWriter(analysis_file, engine='openpyxl') as writer:
            ...
            for anomaly_type, data in anomalies.items():
                if isinstance(data, pd.DataFrame) and len(data) > 0:
                    # Save key columns for readability
                    cols_to_save = ['Name', 'Department_Clean', 'Total Hours_Hours', 'Pay Rate', 'YearWeek']
                    available_cols = [col for col in cols_to_save if col in data.columns]
                    data[available_cols].to_excel(writer, sheet_name=f'Anomaly_{anomaly_type}', index=False)
        ...
        # Save cleaned data
        cleaned_file = f"corrected_timesheet_cleaned_{timestamp}.csv"
    ...
    def run_full_analysis(self):
        ...
        print("🚀 CORRECTED ESKER LODGE TIMESHEET ANALYSIS")
        print("=" * 70)
...
def main():
    ...
    analyzer = CorrectedTimesheetAnalyzer()
